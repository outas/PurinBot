from hoshino import Service, Privilege as Priv

sv = Service('_help_', manage_priv=Priv.SUPERUSER, visible=False)

MANUAL = '''
=====================
- 机械布丁酱使用说明 -
=====================
输入方括号[]内的关键词即可触发相应的功能
※注意其中的【空格】不可省略！
※部分功能必须手动at布丁(或者直接使用昵称)才会触发(复制无效)
※可以识别的昵称：[布丁酱],[布丁],[宫子],[宫子酱],[布丁布丁],[布丁 布丁],[出云宫子]
※※调教时请注意使用频率，您的滥用可能会导致布丁酱账号被封禁
===从此开始↓一行距===

==================
- 公主连接Re:Dive -
==================
[@机械布丁酱 来发十连] 十连转蛋模拟
[@机械布丁酱 来发单抽] 单抽转蛋模拟
[@机械布丁酱 来一井] 4w5钻！买定离手！
[@机械布丁酱 妈] 给主さま盖章章
[查看卡池] 查看机械布丁酱现在的卡池及出率
[怎么拆 妹弓] 后以空格隔开接角色名，查询竞技场解法
[pcr速查] 常用网址/速查表
[bcr速查] B服萌新攻略
[rank表] 查看rank推荐表
[黄骑充电表] 查询黄骑1动充电规律
[@机械布丁酱 官漫132] 官方四格阅览
[禁用 pcr-twitter] 禁用日服官推转发
[启用 pcr-arena-reminder-jp] 背刺时间提醒(UTC+9)
[启用 pcr-arena-reminder-tw] 背刺时间提醒(UTC+8)
[挖矿 15001] 查询矿场中还剩多少钻
[切噜一下] 后以空格隔开接想要转换为切噜语的话
[切噜～♪切啰巴切拉切蹦切蹦] 切噜语翻译
[!帮助] 查看会战管理功能的说明
===========
- 通用功能 -
===========
[启用 bangumi] 开启番剧更新推送
- [@机械布丁酱 来点新番] 查看最近的更新(↑需先开启番剧更新推送↑)
[.r] 掷骰子
[.r 3d12] 掷3次12面骰子
[@机械布丁酱 精致睡眠] 8小时精致睡眠(机械布丁酱需具有群管理权限)
[给我来一份精致昏睡下午茶套餐] 叫一杯先辈特调红茶(机械布丁酱需具有群管理权限)
[@机械布丁酱 来杯咖啡] 联系维护组，空格后接反馈内容
=================
- 群管理限定功能 -
=================
[翻译 もう一度、キミとつながる物語] 机器翻译
[lssv] 查看功能模块的开关状态
========
※除帮助中写明外 另有其他隐藏功能:)

※※调教时请注意使用频率，您的滥用可能会导致机械布丁酱账号被封禁
'''.strip()

@sv.on_command('help', aliases=('manual', '帮助', '说明', '使用说明', '幫助', '說明', '使用說明', '菜单', '菜單'), only_to_me=False)
async def send_help(session):
    await session.send(MANUAL)
